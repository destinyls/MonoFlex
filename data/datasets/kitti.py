...
class KITTIDataset(Dataset):
    ...
    def generate_corners3d(self, ry, dims, locs):
        ...
        corners3d = corners3d + np.array([x, y, z]).reshape([1, 3])
        return corners3d

    def project_rect_to_image(self, P, pts_3d_rect):
    ...
    def __getitem__(self, idx):
        use_right = False
        if self.is_train:
            img, P2, annos, affine, use_right, center_size, image_idx= self.aug_dataset[idx]
        else:
            img, P2, center_size, image_idx = self.aug_dataset[idx]
        original_idx = "{:06d}".format(image_idx)
        calib = self.get_calibration(idx, use_right_cam=use_right)
        ...
        for i, a in enumerate(annos):
            a = a.copy()
            cls_id = a["label"]
            float_occlusion = float(a["occluded"])   # 0 for normal, 0.33 for partially, 0.66 for largely, 1 for unknown (mostly very far and small objs)
            float_truncation = float(a["truncated"]) # 0 ~ 1 and stands for truncation level
            locs = a["loc"]
            dims = a["dim"]                          # l, h, w = dims[0], dims[1], dims[2]
            corners_3d = self.generate_corners3d(a["roty"], dims, locs)
            corners_2d, _ = self.project_rect_to_image(a["P"], corners_3d)
            box2d = np.array([corners_2d[:, 0].min(), corners_2d[:, 1].min(),
                                        corners_2d[:, 0].max(), corners_2d[:, 1].max()])
            box2d[[0,2]] = np.clip(box2d[[0,2]], 0, img_w - 1)
            box2d[[1,3]] = np.clip(box2d[[1,3]], 0, img_h - 1)

            locs[1] = locs[1] - dims[1] / 2
            ...
            if self.filter_annos:
                if float_truncation >= self.filter_params[0] and (box2d[2:] - box2d[:2]).min() <= self.filter_params[1]:
                    continue
            proj_center, depth = self.project_rect_to_image(a["P"], locs.reshape(-1, 3))
            ...
            if (bbox_dim > 0).all() and (0 <= target_center[0] <= self.output_width - 1) and (0 <= target_center[1] <= self.output_height - 1):
                ...
                # 2D bboxes
                gt_bboxes[i] = box2d.copy() # for visualization
